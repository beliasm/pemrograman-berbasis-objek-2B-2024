/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Main.java to edit this template
 */
package pkg230441100110_beliaekasukmamentari;

/**
 *
 * @author asus
 */
import java.util.Scanner;

/**
 *
 * @author asus
 */
public class Main {
    private static int jumlahMedia = 0;

    public static void tambahMedia(Media media) {
        jumlahMedia++;
    }

    public static int getJumlahMedia() {
        return jumlahMedia;
    }

    public static void main(String[] args) {
        Scanner scanner = new Scanner(System.in);

        System.out.println("Masukkan informasi buku:");
        System.out.print("Judul: ");
        String judul = scanner.nextLine();
        System.out.print("Tahun Terbit: ");
        int tahunTerbit = scanner.nextInt();
        System.out.print("Harga: Rp");
        double harga = scanner.nextDouble();

        Buku buku1 = new Buku(judul, tahunTerbit, harga);
        tambahMedia(buku1);

        System.out.println("\nInformasi Buku:");
        System.out.println(buku1.getInfo());
        System.out.println("Total media: " + getJumlahMedia());
        
        System.out.println("\nMasukkan jumlah hari terlambat pengembalian buku pertama : ");
        int hariTerlambat = scanner.nextInt();
        double denda1 = buku1.hitungDenda(hariTerlambat);
        System.out.println("Denda yang harus dibayar untuk buku pertama: Rp" + denda1);
    }
}
